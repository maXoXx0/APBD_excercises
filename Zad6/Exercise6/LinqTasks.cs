...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = from s in Emps
                                      where s.Job.Equals("Backend programmer")
                                      select s;
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.Where(e => e.Salary.Equals(Emps.Max(s => s.Salary)));
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(e => e.Job, (key, emps) => new{Praca = key, LiczbaPracownikow = emps.Count() });
            return result;
        }
        ...
        {
            Emp result = (Emp)Emps.Where(e => e.Job.Equals("Frontend programmer"))
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(Emps.Select(e => new { Ename = "Brak wartości", Job = string.Empty, HireDate = (DateTime?)null} ));
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (e, d) => new { DName = d.Dname, emp = e}).GroupBy(d => d.DName).Where(g => g.Count() > 1).Select(g => new { name = g.Key, numOfEmployees = g.Count()});
            return result;
        }
        ...
        {
            int result = 0;
            result= arr.GroupBy(n => n).Single(n => n.Count() % 2 == 1).Key;
            return result;
        }
